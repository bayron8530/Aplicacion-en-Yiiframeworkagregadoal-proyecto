...
class Gasto extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('monto, etiqueta, fecha', 'required'),
			array('monto', 'length', 'max'=>10),
			array('etiqueta', 'length', 'max'=>25),
			array('descripcion', 'length', 'max'=>125),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('idGasto, monto, etiqueta, fecha, descripcion', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('idGasto',$this->idGasto,true);
